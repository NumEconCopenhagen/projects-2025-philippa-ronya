...
class ConsumerClass:
    ...
    def utility(self,x1,x2,x3):
        ...
        # a. the travel composite
        xB = self.ces(x2,x3,par.beta,par.sigma_B)

        # b. the overall utility (food + travel composite)
    ...
    def solve_grid(self,N=200,do_print=True):
        ...
        # a. create vectors for s1 and w
        s1_vec = np.linspace(0,1,N)
        w_vec = np.linspace(0,1,N)

        # b. create the two-dimensional grid
        ...
        # c. calculate utility at every grid point
        u_grid = self.value_of_choice(s1_grid,w_grid)

        # d. find where utility is highest
        i,j = np.unravel_index(
            np.argmax(u_grid),
            u_grid.shape
        )

        # get the corresponding s1 and w
        s1 = s1_grid[i,j]
        w = w_grid[i,j]
        ...
        opt.s1, opt.s2, opt.s3 = self.shares(s1,w)
        opt.u = u
    ...
    def solve(self,s0=None,do_print=True,**kwargs):
        ...
        res = optimize.minimize(
            self.objective, s0, method='L-BFGS-B', bounds=bounds, callback=callback, **kwargs)
